...
public class ROSNodeService extends AbstractNodeMain implements NodeMain {
    ...
    public void getOptimizedTrajectories(
            final IROSCallback dc) {
        callback = dc;
        preview = true;
    }

    public Trajectory compressArrays(
            final Trajectory old) {
        float siddarth = 1;
        float x1, x2 = 0;
        float y1, y2 = 0;
        float slope1 = 0;
...
//                    slope1 = (y2 - y1)/(x2 - x1);
    ...
    class QuadServiceResponseListener<MessageType> implements ServiceResponseListener<minSnapStampedResponse> {
        ...
        public QuadServiceResponseListener(
                String name,
                DefaultCallback callback) {
            this.name = name;
